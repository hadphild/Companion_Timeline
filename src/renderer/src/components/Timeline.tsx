...
function assignLanes<T extends { id: string; delay: number; action: string }>(
  ...
  for (const a of [...actions].sort((x, y) => x.delay - y.delay)) {
    ...
    for (let lane = 0; lane < laneEdge.length; lane++) {
      if (x >= laneEdge[lane]) { laneEdge[lane] = right; result.push({ id: a.id, lane }); placed = true; break }
    }
...
export default function Timeline({
  ...
  const selectedTracks = useMemo(() => {
    ...
    const ordered = [
      ...
      ...allTriggers.filter(t => !STANDARD_TRIGGERS.includes(t as TriggerKey)).sort((a, b) => {
  ...
  const handleResizeMouseDown = useCallback(
    (e: React.MouseEvent, nextAction: CompanionAction, minDelay: number, triggerKey: TriggerKey) => {
      ...
      resizeRef.current = { nextActionId: nextAction.id, stepKey: currentStepKey, triggerKey, startNextDelay: nextAction.delay, minDelay, startX: e.clientX, containerWidth: w }
      const ppm = pxPerMs(w)
      const onMove = (ev: MouseEvent) => {
        if (!resizeRef.current) return
        const newDelay = Math.max(resizeRef.current.minDelay, snapToGrid(resizeRef.current.startNextDelay + (ev.clientX - resizeRef.current.startX) / ppm, snapMs))
        onActionDelayChange(resizeRef.current.stepKey, resizeRef.current.triggerKey, resizeRef.current.nextActionId, newDelay)
      }
  ...
  const handleActionMouseDown = useCallback(
    (e: React.MouseEvent, action: CompanionAction, triggerKey: TriggerKey) => {
      ...
      const onMove = (ev: MouseEvent) => {
        ...
        onActionMove(dragRef.current.stepKey, dragRef.current.triggerKey, dragRef.current.actionId, Math.max(0, snapToGrid(dragRef.current.startDelay + (ev.clientX - dragRef.current.startX) / ppm, snapMs)))
      }
  ...
  const handlePlayheadDragStart = useCallback((e: React.MouseEvent) => {
    ...
    if (!rulerRect) return
    const onMove = (ev: MouseEvent) => onPlayheadChange(Math.max(0, snapToGrid(pxToMs(ev.clientX - rulerRect.left, w), snapMs)))
  ...
  const handleDrop = useCallback((e: React.DragEvent, triggerKey: TriggerKey) => {
    ...
    const w = (trackAreaRef.current?.clientWidth ?? 900) - LABEL_WIDTH
    const rect = (e.currentTarget as HTMLElement).getBoundingClientRect()
    onActionDrop(currentStepKey, triggerKey, Math.max(0, snapToGrid(pxToMs(e.clientX - rect.left, w), snapMs)), template)
  ...
  const renderRuler = () => {
    ...
    while (t <= scrollMs + visibleMs) {
      ...
      if (x >= 0 && x <= containerWidth) {
        ticks.push(<div key={t} className="ruler-tick" style={{ left: x }}><span className="ruler-label">{msToLabel(t)}</span></div>)
      }
      t += snapMs
    }
    return ticks
      ...
      <div key={triggerKey} className="track-row" style={{ height: trackHeight }}>
        <div className="track-label" style={{ width: LABEL_WIDTH }}>
          ...
          <div className="track-label-meta">
            ...
            {isHoldTrack && (
              <button className="track-remove-btn" title="Remove hold group" onClick={() => onTriggerRemove(currentStepKey, triggerKey)}>×</button>
            )}
          </div>
        </div>
        <div
                ...
                >
                    ...
                    <>
                      ...
                      <span className="action-exec-mode" style={{ display: 'contents' }}>
                        <select
                          ...
                          onChange={e => { e.stopPropagation(); onExecutionModeChange(currentStepKey, triggerKey, e.target.value as ExecutionMode) }}
                        >
                  ...
                  {!hasWait && next && (
                    <button
                      ...
                      onMouseDown={e => e.stopPropagation()}
                      onClick={e => { e.stopPropagation(); setEditingWaitId(action.id); setWaitInput('500') }}
                    >⏱+</button>
                  )}
                </div>
                  ...
                  <div className="clip-wait" style={{ width: wp }} onMouseDown={e => e.stopPropagation()}>
                    {isEditingWait ? (
                      <div className="clip-wait-editor" onMouseDown={e => e.stopPropagation()}>
                        <input
                          className="wait-input" type="number" min={0} step={50} value={waitInput} autoFocus
                          onChange={e => setWaitInput(e.target.value)}
                          onKeyDown={e => {
                            if (e.key === 'Enter') { const ms = parseInt(waitInput); if (!isNaN(ms) && ms >= 0 && next) onActionDelayChange(currentStepKey, triggerKey, next.id, action.delay + ms); setEditingWaitId(null) }
                      ...
                      <button className="clip-wait-label" title="Click to edit wait duration" onClick={e => { e.stopPropagation(); setEditingWaitId(action.id); setWaitInput(String(waitMs)) }}>
                        <span>⏱</span>
                        <span>{msToLabel(waitMs)}</span>
                      </button>
                    )}
                    {next && <div className="clip-resize-handle" onMouseDown={e => handleResizeMouseDown(e, next, action.delay, triggerKey)} />}
                  </div>
                )}
    ...
    <div className="timeline-wrap">
      ...
      {selectedControl && (
        <div className="step-bar">
          ...
          {stepKeys.map((sk) => (
            <button key={sk} className={`step-tab ${sk === currentStepKey ? 'step-tab--active' : ''}`} onClick={() => setActiveStepKey(sk)}>
              {parseInt(sk) + 1}
            </button>
          ))}
          <button className="step-tab step-tab--add" onClick={onStepAdd} title="Add step">+</button>
          {stepKeys.length > 1 && (
            <button className="step-tab step-tab--remove" onClick={() => onStepRemove(currentStepKey)} title="Remove current step">−</button>
          )}
          ...
          {!addingHold ? (
            <button className="step-tab step-tab--add" title="Add hold duration group" onClick={() => setAddingHold(true)}>+</button>
          ) : (
            <div className="hold-input-group">
              <input
                className="hold-input" type="number" min={100} step={100} value={holdInput}
                onChange={e => setHoldInput(e.target.value)}
                onKeyDown={e => {
                  if (e.key === 'Enter') { const ms = parseInt(holdInput); if (!isNaN(ms) && ms >= 100) { onTriggerAdd(currentStepKey, ms); setAddingHold(false); setHoldInput('2000') } }
                  else if (e.key === 'Escape') setAddingHold(false)
                }}
                autoFocus
              />
              <span className="hold-input-unit">ms</span>
              <button className="step-tab step-tab--add" onClick={() => { const ms = parseInt(holdInput); if (!isNaN(ms) && ms >= 100) { onTriggerAdd(currentStepKey, ms); setAddingHold(false); setHoldInput('2000') } }}>✓</button>
      ...
      {contextMenu && (
        ...
        >
          <button
            ...
            onClick={() => { onActionDelete(contextMenu.stepKey, contextMenu.triggerKey, contextMenu.actionId); setContextMenu(null) }}
