...
import { CompanionAction, CompanionInstance, TriggerKey, triggerLabel } from '../types'
...
export default function ActionInspector({
  ...
  const [waitInput, setWaitInput] = useState<string>(
    waitAfterMs != null && waitAfterMs > 0 ? String(waitAfterMs) : ''
  )
  ...
  // Filter action suggestions to those from the selected instance
  const actionSuggestions = useMemo(() => {
    if (!action.instance) return knownActionIds
    return knownActionIds.filter(id => {
      // If the id looks like "instanceLabel:actionId" keep that format
      return true
    })
  }, [knownActionIds, action.instance])

  const listId = `action-list-${action.id}`
  const instanceListId = `instance-list-${action.id}`

  return (
    <div className="inspector">
      <div className="inspector-header">
        <span className="inspector-title">Action</span>
        <button className="inspector-delete" onClick={onDelete} title="Delete action">✕</button>
      </div>
          ...
          <div className="inspector-row">
            ...
            <div className="inspector-input-row">
              <input
                ...
                onKeyDown={e => { if (e.key === 'Enter') handleAddAfter() }}
              />
              <button
            ...
            <div className="inspector-row">
              ...
              <div className="inspector-input-row">
                <input
                  ...
                  value={waitInput}
                  onChange={e => setWaitInput(e.target.value)}
                  onKeyDown={e => { if (e.key === 'Enter') handleWaitApply() }}
                />
                <button
                  className="modal-btn modal-btn--primary"
                  style={{ padding: '4px 10px', fontSize: 12 }}
      ...
      <div className="inspector-section">
        ...
        <div className="inspector-row inspector-option-add-row">
          <input
            ...
            onKeyDown={e => { if (e.key === 'Enter') commitNewOption() }}
          />
          <button
            className="modal-btn modal-btn--primary"
            style={{ padding: '4px 10px', fontSize: 12 }}
